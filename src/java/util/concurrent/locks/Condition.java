...
/*
    ...
    都需要和锁进行关联：
        对象监视器: 需要进入synchronized语句块（进入对象监视器）才能调用对象监视器的方法（Object.wait()）。
 ...
 */
    ...
    Condition的强大之处在于，对于一个锁，我们可以为多个线程间建立不同的Condition。如果采用Object类中的wait(), notify(), notifyAll()实现的话，
    当写入数据之后需要唤醒读线程时，不可能通过notify()或notifyAll()明确的指定唤醒读线程，而只能通过notifyAll唤醒所有线程，
    但是notifyAll无法区分唤醒的线程是读线程，还是写线程。所以，通过Condition能够更加精细的控制多线程的休眠与唤醒。
     */
public interface Condition {
     ...
     *         (and interruption of thread suspension is supported)
     */
    //调用此方法的线程将加入等待队列，阻塞直到被通知或者线程发生中断或者超出等待的时间(需要执行时间单位)
